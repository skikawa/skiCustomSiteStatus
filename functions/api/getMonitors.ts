...
function getEnvValue(env: RuntimeEnv, key: keyof RuntimeEnv) {
  return env[key] || "";
}
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ code: 200, message: "success", source: "api", data: formattedData }), {
      status: 200,
      headers: { "content-type": "application/json" },
    });
  } catch (err: any) {
    console.error("Error in getMonitors function:", err);
